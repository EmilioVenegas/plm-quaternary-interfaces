...
tests across 4 single-chain foundation model scales and 3D
structure-conditioned ProteinMPNN (\(N = 21,286\) stacked observations,
5 system clusters). Single-chain PLMs exhibit severe selective collapse
(\(\beta_7 \approx -0.334\text{ to }-0.353\)), whereas 3D complex
...
analysis across all four foundation model arms and structural
compartments (\(N = 10,643\) paired variants). Unadjusted raw
correlations across the entire unstacked dataset confirm that partial
binding correlations at interfaces are strictly
negative.}\label{tbl-mediation-table}\tabularnewline
\toprule\noalign{}
\begin{minipage}[b]{\linewidth}\raggedright
Model Arm
\end{minipage} & \begin{minipage}[b]{\linewidth}\raggedright
...
\subsection{\texorpdfstring{A three-way interaction model
...
significantly
negative.}{A three-way interaction model (z(\textbackslash text\{DMS\}) \textbackslash sim \textbackslash text\{PLM\} \textbackslash times \textbackslash text\{Binding\} \textbackslash times \textbackslash text\{Hotspot\}) was estimated on the Hotspot \textbackslash cup Rim subset (N = 1,518) to formally evaluate whether the three-way interaction coefficient \textbackslash beta\_\{(\textbackslash text\{PLM\} \textbackslash times \textbackslash text\{Binding\} \textbackslash times \textbackslash text\{Hotspot\})\} was significantly negative.}}\label{a-three-way-interaction-model-ztextdms-sim-textplm-times-textbinding-times-texthotspot-was-estimated-on-the-hotspot-cup-rim-subset-n-1518-to-formally-evaluate-whether-the-three-way-interaction-coefficient-beta_textplm-times-textbinding-times-texthotspot-was-significantly-negative.}
